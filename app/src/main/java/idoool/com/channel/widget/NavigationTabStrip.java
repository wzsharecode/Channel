 ...
 * @description: 创建 NavigationTabStrip
 */
...
public class NavigationTabStrip extends View implements ViewPager.OnPageChangeListener {
    ...
    private void updateIndicatorPosition(final float fraction) {
        ...
        mStripLeft =
                mStartStripX + (mResizeInterpolator.getResizeInterpolation(fraction, mIsResizeIn) *
                        (mEndStripX - mStartStripX)) + mTabSize / mTitles.length * 6 / 5;
        // Set the strip right side coordinate
        mStripRight =
                (mStartStripX + (mStripType == StripType.LINE ? mTabSize / mTitles.length * 7 / 4 : mStripWeight)) +
    ...
    protected void onDraw(final Canvas canvas) {
        // Set bound of strip
        mStripBounds.set(
                mStripLeft - (mStripType == StripType.POINT ? mStripWeight * 0.5F : 0.5F),
        ...
        for (int i = 0; i < mTitles.length; i++) {
            ...
            canvas.drawText(title, leftTitleOffset, topTitleOffset + (mStripGravity == StripGravity.TOP ? mStripWeight : 0.0F),
                    mTitlePaint
            );
        }
    }
